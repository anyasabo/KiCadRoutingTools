...
from bga_fanout.types import FanoutRoute, create_track
from kicad_parser import PCBData


def detect_collisions(
    tracks: list[dict], existing_tracks: list[dict], min_spacing: float, max_pairs: int = 5
    ...
    for i, t1 in enumerate(tracks):
        ...
        for j, t2 in enumerate(tracks[i + 1 :], i + 1):
            ...
            if t1.get("pair_id") and t1.get("pair_id") == t2.get("pair_id"):
                continue
            if check_segment_collision(t1["start"], t1["end"], t2["start"], t2["end"], min_spacing):
                collision_count += 1
...
def convert_segments_to_tracks(pcb_data: PCBData) -> list[dict]:
    ...
    for seg in pcb_data.segments:
        existing_tracks.append(
            create_track(
                (seg.start_x, seg.start_y), (seg.end_x, seg.end_y), seg.width, seg.layer, seg.net_id, is_existing=True
            )
        )
...
def generate_tracks_from_routes(
    ...
    for route in routes:
        ...
        if getattr(route, "neighbor_connection", False):
            ...
            tracks.append(
                create_track(route.pad_pos, route.exit_pos, track_width, route.layer, route.net_id, route.pair_id)
            )
            neighbor_count += 1
            continue
        ...
        if route.is_edge:
            ...
            if route.pair_id and (
                abs(route.stub_end[0] - route.pad_pos[0]) > POSITION_TOLERANCE
                or abs(route.stub_end[1] - route.pad_pos[1]) > POSITION_TOLERANCE
            ):
                # Differential edge pair: 45° stub to converge, then straight to exit
                # 45° stub: pad -> stub_end (convergence point)
                tracks.append(
                    create_track(route.pad_pos, route.stub_end, track_width, route.layer, route.net_id, route.pair_id)
                )
                # Straight segment: stub_end -> exit
                tracks.append(
                    create_track(route.stub_end, route.exit_pos, track_width, route.layer, route.net_id, route.pair_id)
                )
            else:
            ...
            if route.jog_end:
                if route.jog_extension:
                    ...
                    tracks.append(
                        create_track(
                            route.exit_pos, route.jog_extension, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                    # Then the 45° jog from extension point
                    tracks.append(
                        create_track(
                            route.jog_extension, route.jog_end, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                else:
                    ...
                    tracks.append(
                        create_track(
                            route.exit_pos, route.jog_end, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
            edge_count += 1
        else:
            ...
            if route.channel_point:
                ...
                tracks.append(
                    create_track(
                        route.pad_pos, route.channel_point, track_width, route.layer, route.net_id, route.pair_id
                    )
                )
                ...
                if route.channel_point2:
                    ...
                    tracks.append(
                        create_track(
                            route.channel_point2, route.stub_end, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                else:
                    ...
                    tracks.append(
                        create_track(
                            route.channel_point, route.stub_end, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                # Straight segment: stub_end -> exit
                tracks.append(
                ...
                if has_stub:
                    ...
                    tracks.append(
                        create_track(
                            route.pad_pos, route.stub_end, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                ...
                if route.pre_channel_jog:
                    ...
                    tracks.append(
                        create_track(
                            route.stub_end, route.pre_channel_jog, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                    # Channel segment: pre_channel_jog -> exit
                    tracks.append(
                        create_track(
                            route.pre_channel_jog, route.exit_pos, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
                else:
                    ...
                    tracks.append(
                        create_track(
                            route.stub_end, route.exit_pos, track_width, route.layer, route.net_id, route.pair_id
                        )
                    )
