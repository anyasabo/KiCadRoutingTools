...
def _ssl_help_message():
    ...
    if sys.platform == "darwin":
        msg += (
            "On macOS, run the 'Install Certificates.command' shipped with your "
...
def build_from_source(script_dir, rust_dir):
    ...
    try:
        result = subprocess.run(["cargo", "build", "--release"], cwd=rust_dir, capture_output=False)
...
def main():
    ...
    parser.add_argument("--clean", action="store_true", help="Remove all Rust build outputs and compiled library files")
    parser.add_argument(
        "--from-source", action="store_true", help="Skip the prebuilt download and build locally with cargo"
    )
    parser.add_argument(
        "--tag", default=None, help="Download from a specific release tag (e.g. v0.15.0) instead of the latest release"
    )
