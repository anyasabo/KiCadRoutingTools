...
def get_ignored_files():
    ...
    result = subprocess.run(
        ["git", "ls-files", "--ignored", "--exclude-standard", "-o"], capture_output=True, text=True
    )
...
def get_ignored_dirs():
    ...
    result = subprocess.run(
        ["git", "ls-files", "--ignored", "--exclude-standard", "-o", "--directory"], capture_output=True, text=True
    )
    if result.returncode != 0:
        return []
...
def main():
    ...
    parser.add_argument("--delete", action="store_true", help="Actually delete the files (default: just list them)")
