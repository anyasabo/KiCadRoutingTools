...
from kicad_writer import (
    ...
    generate_gr_line_sexpr,
    generate_gr_text_sexpr,
...
def write_routed_output(
    ...
    if not (
        results
        or all_segment_modifications
        or all_swap_vias
        or target_swap_info
        or single_ended_target_swap_info
        or skip_routing
    ):
        return False
    ...
    content = _apply_single_ended_target_swaps(
        content, single_ended_target_swap_info, net_id_to_name if kicad_v10 else None
    )
    ...
    if debug_lines:
        routing_text += _generate_debug_paths(results, exclusion_zone_lines or [], boundary_debug_labels or [])
...
def _apply_diff_pair_target_swaps(
    ...
    for swap in target_swap_info:
        ...
        content, p1_p_seg = swap_segment_nets_at_positions(
            ...
            swap["p2_p_net_id"],
            layer=p1_layer,
            old_net_name=_net_name(swap["p1_p_net_id"]),
            new_net_name=_net_name(swap["p2_p_net_id"]),
        )
        content, p1_n_seg = swap_segment_nets_at_positions(
            ...
            swap["p2_n_net_id"],
            layer=p1_layer,
            old_net_name=_net_name(swap["p1_n_net_id"]),
            new_net_name=_net_name(swap["p2_n_net_id"]),
        )
        ...
        content, p2_p_seg = swap_segment_nets_at_positions(
            ...
            swap["p1_p_net_id"],
            layer=p2_layer,
            old_net_name=_net_name(swap["p2_p_net_id"]),
            new_net_name=_net_name(swap["p1_p_net_id"]),
        )
        content, p2_n_seg = swap_segment_nets_at_positions(
            ...
            swap["p1_n_net_id"],
            layer=p2_layer,
            old_net_name=_net_name(swap["p2_n_net_id"]),
            new_net_name=_net_name(swap["p1_n_net_id"]),
        )
        ...
        if swap["p1_p_pad"] and swap["p2_p_pad"]:
            print(
                f"    Swapping P pads in output: {swap['p1_p_pad'].component_ref}:{swap['p1_p_pad'].pad_number} <-> {swap['p2_p_pad'].component_ref}:{swap['p2_p_pad'].pad_number}"
            )
        ...
        if swap["p1_n_pad"] and swap["p2_n_pad"]:
            print(
                f"    Swapping N pads in output: {swap['p1_n_pad'].component_ref}:{swap['p1_n_pad'].pad_number} <-> {swap['p2_n_pad'].component_ref}:{swap['p2_n_pad'].pad_number}"
            )
...
def _apply_single_ended_target_swaps(
    ...
    for swap in single_ended_target_swap_info:
        ...
        content, n1_seg = swap_segment_nets_at_positions(
            ...
            swap["n1_net_id"],
            swap["n2_net_id"],
            old_net_name=_net_name(swap["n1_net_id"]),
            new_net_name=_net_name(swap["n2_net_id"]),
        )
        ...
        content, n2_seg = swap_segment_nets_at_positions(
            ...
            swap["n2_net_id"],
            swap["n1_net_id"],
            old_net_name=_net_name(swap["n2_net_id"]),
            new_net_name=_net_name(swap["n1_net_id"]),
        )
        ...
        if swap["n1_pad"] and swap["n2_pad"]:
            print(
                f"    Swapping pads in output: {swap['n1_pad'].component_ref}:{swap['n1_pad'].pad_number} <-> {swap['n2_pad'].component_ref}:{swap['n2_pad'].pad_number}"
            )
...
def _apply_polarity_swaps(content: str, pad_swaps: list[dict], pcb_data, net_id_to_name: dict | None = None) -> str:
    ...
    for swap in pad_swaps:
        ...
        content, p_seg_count = swap_segment_nets_at_positions(
            content, p_positions, p_net_id, n_net_id, old_net_name=_net_name(p_net_id), new_net_name=_net_name(n_net_id)
        )
        content, n_seg_count = swap_segment_nets_at_positions(
            content, n_positions, n_net_id, p_net_id, old_net_name=_net_name(n_net_id), new_net_name=_net_name(p_net_id)
        )
...
def _generate_debug_paths(results: list[dict], exclusion_zone_lines: list, boundary_debug_labels: list) -> str:
    """Generate debug visualization paths."""
    routing_text = ""

    print(
        "Adding debug paths to User.3 (connectors), User.4 (stub dirs), User.5 (exclusion zones), User.8 (simplified), User.9 (raw A*)"
    )
    ...
    if boundary_debug_labels:
        ...
        for label in boundary_debug_labels:
            routing_text += (
                generate_gr_text_sexpr(
                    label["text"], label["x"], label["y"], label["layer"], size=0.1, angle=label.get("angle", 0)
                )
                + "\n"
            )
