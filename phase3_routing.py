...
from obstacle_map import (
    ...
    remove_segments_list_from_obstacles,
    remove_vias_list_from_obstacles,
)
...
from routing_context import build_incremental_obstacles, build_single_ended_obstacles
...
def run_phase3_tap_routing(
    ...
    for net_id, main_result in list(state.pending_multipoint_nets.items()):
        ...
        completed_result = route_multipoint_taps(
            ...
            obstacles,
            tap_input,
            global_offset=global_tap_offset,
            global_total=total_tap_edges,
        ...
        if completed_result:
            ...
            net_edges_attempted = (
                completed_result.get("tap_edges_routed", 0) + completed_result.get("tap_edges_failed", 0) - 1
            )  # -1 for Phase 1 edge
            ...
            if tap_segments or tap_vias:
                ...
                if state.working_obstacles is not None and state.net_obstacles_cache is not None:
                    ...
                    update_net_obstacles_after_routing(
                        pcb_data, net_id, completed_result, config, state.net_obstacles_cache
                    )
...
def try_phase3_ripup(
    ...
    blockers = analyze_frontier_blocking(
        all_blocked_cells, pcb_data, config, routed_net_paths, exclude_net_ids={net_id}, obstacle_cache=obstacle_cache
    )
    ...
    for N in range(1, config.max_rip_up_count + 1):
        ...
        blocker_name = (
            pcb_data.nets[blocker.net_id].name if blocker.net_id in pcb_data.nets else f"net_{blocker.net_id}"
        )
        ...
        for rid in ripped_ids:
            ...
            record_net_event(
                ...
                rid,
                "ripped_by",
                {"ripping_net_id": net_id, "ripping_net_name": net_name, "reason": f"Phase 3 tap rip-up N={N}", "N": N},
            )
            ...
            if retry_failed < original_failed_count:
                ...
                if state.working_obstacles is not None and net_id not in ripped_net_ids:
                    ...
                    if tap_segments_added or tap_vias_added:
                        print(
                            f"    Adding {len(tap_segments_added)} tap segments, {len(tap_vias_added)} tap vias to obstacles before re-routing..."
                        )
                    ...
                    print("    Skipping tap obstacle addition - net will be re-routed")
    ...
    for ripped_net_id, saved_result, ripped_ids, was_in_results in phase3_ripped_nets:
        ...
        if result and not result.get("failed") and result.get("path"):
            ...
            record_net_event(
                ...
                ripped_net_id,
                "reroute_succeeded",
                {"segments": len(result["new_segments"]), "vias": len(main_vias)},
            )
                ...
                if tap_result:
                    ...
                    if (
                        ...
                        if all_blocked_cells:
                            print(
                                f"    {net_name}: Attempting rip-up retry for {original_failed_count} failed tap edge(s)..."
                            )
                ...
                if tap_result:
                    ...
                    if tap_segments or tap_vias:
                        ...
                        if state.working_obstacles is not None and state.net_obstacles_cache is not None:
                            if ripped_net_id in state.net_obstacles_cache:
                                remove_net_obstacles_from_cache(
                                    state.working_obstacles, state.net_obstacles_cache[ripped_net_id]
                                )
                            update_net_obstacles_after_routing(
                                pcb_data, ripped_net_id, tap_result, config, state.net_obstacles_cache
                            )
                            add_net_obstacles_from_cache(
                                state.working_obstacles, state.net_obstacles_cache[ripped_net_id]
                            )
            ...
            record_net_event(
                state, ripped_net_id, "reroute_failed", {"reason": "no path found during Phase 3 re-route"}
            )
