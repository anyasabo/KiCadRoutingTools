...
def apply_polarity_swap(
    ...
    if pad_p and pad_n:
        ...
        print(
            f"  Polarity fixed: will swap nets of {pad_p.component_ref}:{pad_p.pad_number} <-> {pad_n.component_ref}:{pad_n.pad_number}"
        )
        return True
    else:
        print("  WARNING: Could not find target pads to swap for polarity fix")
        if not pad_p:
