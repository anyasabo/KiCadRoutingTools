...
from memory_debug import (
    ...
    estimate_track_proximity_cache_mb,
    format_memory_stats,
    format_obstacle_map_stats,
    get_process_memory_mb,
)
from net_ordering import order_nets_inside_out, order_nets_mps
from net_queries import (
    expand_net_patterns,
    find_single_ended_nets,
    get_all_unrouted_net_ids,
    get_chip_pad_positions,
    identify_power_nets,
)
from obstacle_cache import build_working_obstacle_map, precompute_all_net_obstacles
from obstacle_map import (
    build_base_obstacle_map,
    build_base_obstacle_map_with_vis,
...
from routing_common import (
    filter_already_routed,
    get_common_config_kwargs,
    resolve_net_ids,
    run_length_matching,
    setup_bga_exclusion_zones,
    sync_pcb_data_segments,
)

# Import from refactored modules
from routing_config import GridRouteConfig
from routing_constants import DEFAULT_4_LAYER_STACK, POWER_NET_EXCLUSION_PATTERNS
from routing_state import create_routing_state, print_failed_net_histories
...
def batch_route(
    ...
    swappable_net_patterns: list[str] | None = None,
    ...
    length_match_groups: list[list[str]] | None = None,
    ...
    if pcb_data is None:
        ...
        pcb_data = parse_kicad_pcb(input_file, guide_layer=guide_corridor_layer, keepout_layer=keepout_layer)
    else:
    ...
    if impedance is not None:
        if not pcb_data.board_info.stackup:
            ...
            print_impedance_routing_plan(pcb_data, layers, impedance, is_differential=False, min_width=track_width)
    ...
    if config.corridor_waypoints:
        print(
            f"Guide corridor: steering routes through {len(config.corridor_waypoints)} "
    ...
    if config.keepout_enabled and pcb_data.keepout_zones:
        print(f"Keepout: blocking routes from {len(pcb_data.keepout_zones)} polygon(s) on {config.keepout_layer}")

    # Identify power nets and set up per-net widths
    if power_nets and power_nets_widths:
        if len(power_nets) != len(power_nets_widths):
            raise ValueError(
                f"--power-nets ({len(power_nets)}) and --power-nets-widths ({len(power_nets_widths)}) must have same length"
            )
    ...
    if not net_ids:
        ...
        if return_results:
            return (
                ...
                0,
                0.0,
                {"results": [], "all_swap_vias": [], "exclusion_zone_lines": [], "boundary_debug_labels": []},
            )
        return 0, 0, 0.0
    ...
    if swappable_net_patterns:
        ...
        swappable_se_nets = find_single_ended_nets(pcb_data, swappable_net_patterns, exclude_net_ids=set())
    ...
    if enable_layer_switch and net_ids:
        total_layer_swaps += apply_single_ended_layer_swaps(
            pcb_data, config, net_ids, can_swap_to_top_layer, all_segment_modifications, all_swap_vias, verbose=verbose
        )
    ...
    if visualize:
        ...
        base_obstacles = build_base_obstacle_map(pcb_data, config, all_net_ids_to_route, net_clearances=net_clearances)
    ...
    if debug_lines:
        ...
        print(
            f"Will draw {len(config.bga_exclusion_zones)} BGA zones and {len(all_proximity_points)} stub/pad proximity circles on User.5"
        )
    ...
    for net_id, result in routed_results.items():
        if result.get("is_multipoint"):
            ...
            if failed_pads_info:
                net_name = pcb_data.nets[net_id].name if net_id in pcb_data.nets else f"Net {net_id}"
                failed_multipoint.append({"net_name": net_name, "net_id": net_id, "failed_pads": failed_pads_info})
    ...
    if single_ended_nets:
        if failed_single:
            print(
                f"  {RED}Single-ended:  {len(routed_single)}/{len(single_ended_nets)} routed ({len(failed_single)} FAILED){RESET}"
            )
        else:
    ...
    if multipoint_nets > 0:
        ...
        if tap_pads_failed > 0:
            print(
                f"  {RED}Multi-point:   {tap_pads_connected}/{tap_pads_total} pads connected ({tap_pads_failed} FAILED){RESET}"
            )
        else:
    ...
    if failed_multipoint:
        ...
        for item in failed_multipoint:
            ...
            for pad in item["failed_pads"]:
                print(
                    f"  {RED}{net_name}: {pad['component_ref']} pad {pad['pad_number']} at ({pad['x']:.2f}, {pad['y']:.2f}) not connected{RESET}"
                )
    ...
    if debug_memory:
        ...
        print(
            f"  Net obstacles cache: {estimate_net_obstacles_cache_mb(state.net_obstacles_cache):.1f} MB ({len(state.net_obstacles_cache)} nets)"
        )
        print(
            f"  Track proximity cache: {estimate_track_proximity_cache_mb(state.track_proximity_cache):.1f} MB ({len(state.track_proximity_cache)} nets)"
        )
        print(
            f"  Routed paths: {estimate_routed_paths_mb(state.routed_net_paths):.1f} MB ({len(state.routed_net_paths)} nets)"
        )
...
""",
    ...
    parser.add_argument(
        "net_patterns", nargs="*", help="Net names or wildcard patterns to route (default: '*' = all nets)"
    )
    parser.add_argument(
        "--nets", "-n", nargs="+", help="Net names or wildcard patterns to route (alternative to positional args)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--component",
        "-C",
        help="Route all nets connected to this component (e.g., U1). Excludes GND/VCC/VDD unless net patterns also specified.",
    )
    # Ordering and strategy options
    parser.add_argument(
        "--ordering",
        "-o",
        choices=["inside_out", "mps", "original"],
    ...
    parser.add_argument(
        "--layers", "-l", nargs="+", default=defaults.DEFAULT_LAYERS, help="Routing layers to use (default: F.Cu B.Cu)"
    )
    ...
    parser.add_argument(
        "--turn-cost",
        type=int,
        default=defaults.TURN_COST,
        help=f"Penalty for direction changes, encourages straighter paths (default: {defaults.TURN_COST})",
    )
    parser.add_argument(
        "--direction-preference-cost",
        type=int,
        default=defaults.DIRECTION_PREFERENCE_COST,
    ...
    parser.add_argument(
        "--guide-corridor-layer",
        type=str,
        default=defaults.GUIDE_CORRIDOR_LAYER,
        help=f"User layer the guide polyline is drawn on (default: {defaults.GUIDE_CORRIDOR_LAYER})",
    )
    parser.add_argument(
        "--guide-corridor-spacing",
        type=float,
        default=defaults.GUIDE_CORRIDOR_SPACING,
    ...
    parser.add_argument(
        "--keepout-layer",
        type=str,
        default=defaults.KEEPOUT_LAYER,
    ...
    parser.add_argument(
        ...
        type=float,
        default=defaults.TRACK_PROXIMITY_DISTANCE,
    ...
    parser.add_argument(
        ...
        type=float,
        default=defaults.TRACK_PROXIMITY_COST,
    ...
    parser.add_argument(
        "--mps-layer-swap", action="store_true", help="Enable MPS-aware layer swaps to reduce crossing conflicts"
    )
    parser.add_argument(
        "--mps-segment-intersection",
        action="store_true",
    ...
    parser.add_argument(
        "--time-match-tolerance", type=float, default=1.0, help="Acceptable time variance in picoseconds (default: 1.0)"
    )
    ...
    parser.add_argument(
        "--vertical-attraction-radius",
        type=float,
        default=1.0,
        help="Radius in mm for cross-layer track attraction (0 = disabled, default: 1.0)",
    )
    parser.add_argument(
        "--vertical-attraction-cost",
        type=float,
        default=defaults.VERTICAL_ATTRACTION_COST,
    ...
    parser.add_argument(
        "--ripped-route-avoidance-radius",
        type=float,
        default=defaults.RIPPED_ROUTE_AVOIDANCE_RADIUS,
        help=f"Radius in mm around ripped route segments/vias for soft penalty (default: {defaults.RIPPED_ROUTE_AVOIDANCE_RADIUS})",
    )
    parser.add_argument(
        "--ripped-route-avoidance-cost",
        type=float,
        default=defaults.RIPPED_ROUTE_AVOIDANCE_COST,
    ...
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="Print detailed diagnostic output (setback checks, etc.)"
    )
    parser.add_argument(
        "--skip-routing", action="store_true", help="Skip actual routing, only do swaps and write debug info"
    )
    parser.add_argument(
        "--debug-memory", action="store_true", help="Print memory usage statistics at key points during routing"
    )
    parser.add_argument("--add-teardrops", action="store_true", help="Add teardrop settings to all pads in output file")
    parser.add_argument(
        "--stats", action="store_true", help="Collect and print A* search statistics for debugging heuristic efficiency"
    )

    # Visualization options
    parser.add_argument(
        "--visualize", "-V", action="store_true", help="Show real-time visualization of the routing (requires pygame)"
    )
    parser.add_argument(
        "--auto", action="store_true", help="Auto-advance to next net without waiting (with --visualize)"
    )
    parser.add_argument(
        "--display-time",
        type=float,
    ...
    if args.visualize:
        try:
            ...
            vis_callback = create_pygame_callback(
                layers=args.layers, auto_advance=args.auto, display_time=args.display_time
            )
