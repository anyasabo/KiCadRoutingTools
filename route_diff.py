...
from diff_pair_routing import get_diff_pair_connector_regions
...
from net_ordering import order_nets_inside_out, order_nets_mps, separate_nets_by_type
...
from routing_state import create_routing_state
...
def batch_route_diff_pairs(
    ...
    if impedance is not None:
        if not pcb_data.board_info.stackup:
            ...
            layer_widths = calculate_layer_widths_for_impedance(
                ...
                layers,
                impedance,
                spacing=diff_pair_gap,
                is_differential=True,
                fallback_width=track_width,
                min_width=track_width,
            )
            print_impedance_routing_plan(
                pcb_data, layers, impedance, spacing=diff_pair_gap, is_differential=True, min_width=track_width
            )
    ...
    if not diff_pairs:
        print("Error: No differential pairs found matching the patterns!")
    ...
    if swappable_net_patterns and diff_pair_ids_to_route_set:
        ...
        swappable_pairs = [(name, pair) for name, pair in swappable_diff_pairs.items() if name in pairs_being_routed]
    ...
    if debug_lines and mps_unroll and not boundary_debug_labels and diff_pair_ids_to_route_set:
        ...
        boundary_debug_labels = generate_debug_boundary_labels(
            pcb_data,
            list(diff_pair_ids_to_route_set),
            lambda pair: get_diff_pair_endpoints(pcb_data, pair.p_net_id, pair.n_net_id, config),
        )
    ...
    if enable_layer_switch and diff_pair_ids_to_route_set:
        total_layer_swaps, all_stubs_by_layer, stub_endpoints_by_layer = apply_diff_pair_layer_swaps(
            pcb_data,
            config,
            diff_pair_ids_to_route_set,
            diff_pairs,
            can_swap_to_top_layer,
            all_segment_modifications,
            all_swap_vias,
        )
    ...
    diff_pair_ids_to_route, single_ended_nets = separate_nets_by_type(net_ids, diff_pairs, diff_pair_net_ids)
    ...
    diff_pair_base_obstacles = build_base_obstacle_map(
        pcb_data, config, all_net_ids_to_route, diff_pair_extra_clearance
    )
        ...
        for pair_name, pair in diff_pair_ids_to_route:
            ...
            if connector_info:
                ...
                add_connector_region_via_blocking(
                    ...
                    connector_info["src_center"][0],
                    connector_info["src_center"][1],
                    connector_info["src_dir"][0],
                    connector_info["src_dir"][1],
                    connector_info["src_setback"],
                    connector_info["spacing_mm"],
                    config,
                )
                # Block target connector region
                add_connector_region_via_blocking(
                    diff_pair_base_obstacles,
                    connector_info["tgt_center"][0],
                    connector_info["tgt_center"][1],
                    connector_info["tgt_dir"][0],
                    connector_info["tgt_dir"][1],
                    connector_info["tgt_setback"],
                    connector_info["spacing_mm"],
                    config,
                )
    ...
    dp_successful, dp_failed, dp_time, dp_iterations, route_index = route_diff_pairs(state, diff_pair_ids_to_route)
    ...
    if failed_diff_pairs:
        print(
            f"  {RED}Diff pairs:    {len(routed_diff_pairs)}/{len(diff_pair_ids_to_route)} routed ({len(failed_diff_pairs)} FAILED){RESET}"
        )
    else:
...
""",
    ...
    parser.add_argument(
        "--layers", "-l", nargs="+", default=["F.Cu", "B.Cu"], help="Routing layers to use (default: F.Cu B.Cu)"
    )
    ...
    parser.add_argument("--clearance", type=float, default=0.25, help="Clearance between tracks in mm (default: 0.25)")
    ...
    parser.add_argument(
        "--max-iterations", type=int, default=200000, help="Max A* iterations before giving up (default: 200000)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--keepout", action="store_true", help="Keep routed tracks out of polygons drawn on a User layer (issue #27)"
    )
    parser.add_argument(
        "--keepout-layer",
        type=str,
    ...
    parser.add_argument(
        "--stub-proximity-cost", type=float, default=0.2, help="Cost penalty near stubs in mm equivalent (default: 0.2)"
    )
